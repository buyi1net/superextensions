...
信息路径为新开一个 `Agent` 会话,不是新开一个子代理,不要偷懒,不要错判,分清楚下面列出的运行环境后直接启动一个全新的 `Agent` 会话执行交接即可。
...
任务交出去不等于交接完成,要等新会话确认无误后才算彻底交接完毕.所以正确的顺序应该是:

1. 新开会话后等待新会话验证信息.
2. 如果新会话发现信息不符,要给它发消息帮助新会话确认信息.
3. 新会话确认无误后,才算是正式完成工作交接.
